...
impl<P: Default + Copy> TwoDimensionalMap<P> {
	...
	pub fn get_mut_row(&mut self, y: u32) -> &mut [P] {
		return &mut self.energy[((y * self.width) as usize)..(((y + 1) * self.width) as usize)];
	}

	pub fn get_row(&self, y: u32) -> &[P] {
		return &self.energy[((y * self.width) as usize)..(((y + 1) * self.width) as usize)];
	}
}
